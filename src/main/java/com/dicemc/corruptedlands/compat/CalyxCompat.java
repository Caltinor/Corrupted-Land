package com.dicemc.corruptedlands.compat;
...
public class CalyxCompat {
	...
	public static boolean calyxDetected(Player player, ResourceLocation blockID) {
	...
	public static boolean corrupt(ItemStack item) {
		return calyxCap != null && Config.CALYX_EGGS_CORRUPT_LAND.get() && item.sameItem(new ItemStack(Register.IMMORTUOSCALYXEGGS.get()));
	}
	
	public static boolean isInfected(LivingEntity entity, Block block) {
		return calyxCap != null && entity instanceof InfectedEntity && block instanceof ICorrupted;
	}
